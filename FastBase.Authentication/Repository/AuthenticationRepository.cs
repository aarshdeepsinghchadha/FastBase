...
namespace FastBase.Authentication.Repository
{
    public class AuthenticationRepository : IAuthenticationRepository
    {
        ...
        public AuthenticationRepository(IResponseGeneratorService responseGeneratorService, UserManager<AppUser> userManager, SignInManager<AppUser> signInManager, ITokenService tokenService, IEmailSenderService emailSender, ILogger<AuthenticationRepository> logger, RoleManager<IdentityRole> roleManager, DataContext context, IMapper mapper)
        {
        ...
        public async Task<ReturnResponse> RegisterUserAsync(RegisterDto registerDto, string origin)
        {
            try
            {
                ...
                var roleName = SystemEnums.Roles.User.ToString();
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        return await _responseGeneratorService.GenerateResponseAsync(
                            true, StatusCodes.Status200OK, $"User registered successfully. Please check your email and verify your email address.");
                    }
                    else
                    {
                        return await _responseGeneratorService.GenerateResponseAsync(
                           true, StatusCodes.Status400BadRequest, $"User registered successfully. Email not sent due to some technical issues: {sendVerificationEmailResult.Message}. Please click on the Resent Verification Link to continue. If the error continues, please contact support.");
                    }
                }
                else
                {
